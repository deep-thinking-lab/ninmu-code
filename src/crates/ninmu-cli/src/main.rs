...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("ninmu-cli-{nanos}-{unique}"))
    }
...
UU conflicted.rs",
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading src/crates/ninmu-cli/src/main.rs".to_string()),
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p ninmu-cli"}"#)
                .contains("cargo test -p ninmu-cli")
        );
        assert_eq!(
            describe_tool_progress("grep_search", r#"{"pattern":"ultraplan","path":"src"}"#),
            "grep `ultraplan` in src"
        );
    }
    ...
    fn response_to_events_preserves_non_empty_json_input_outside_streaming() {
        ...
        let events = response_to_events(
            MessageResponse {
                ...
                content: vec![OutputContentBlock::ToolUse {
                    id: "tool-2".to_string(),
                    name: "read_file".to_string(),
                    input: json!({ "path": "src/Cargo.toml" }),
                }],
                stop_reason: Some("tool_use".to_string()),
                stop_sequence: None,
                usage: Usage {
        ...
        assert!(matches!(
            &events[0],
            AssistantEvent::ToolUse { name, input, .. }
                if name == "read_file" && input == "{\"path\":\"src/Cargo.toml\"}"
        ));
    }
